...
# STRICT ASSET ROSTER (yfinance ticker -> MT5 Symbol Name)
WEEKDAY_ASSETS = {
    "GC=F": "XAUUSD",             # Gold Futures / MT5 Gold symbol
    "EURUSD=X": "EURUSD",
    "GBPUSD=X": "GBPUSD",
    "JPY=X": "USDJPY",
    "AUDUSD=X": "AUDUSD",
...
last_signals = {}
active_open_trades = {}  # Tracks active trades for dynamic management
...
@flask_app.route('/')
def home():
    return "Kings™ MT5 Auto Engine with Active Risk Management is Live!"
...
def execute_mt5_trade(symbol_label, action, volume=0.01, sl_price=0.0, tp_price=0.0):
    """Executes market orders directly on MetaTrader 5 terminal."""
    if not mt5.initialize():
        logging.error(f"MT5 initialization failed, error code = {mt5.last_error()}")
        return None

    # Log in to account if credentials are provided
    if MT5_LOGIN != "YOUR_MT5_ACCOUNT_NUMBER":
        authorized = mt5.login(login=MT5_LOGIN, password=MT5_PASSWORD, server=MT5_SERVER)
        if not authorized:
            logging.error(f"MT5 failed to connect to account {MT5_LOGIN}, error code = {mt5.last_error()}")
            mt5.shutdown()
            return None
    ...
    if symbol_info is None:
        logging.warning(f"Symbol {symbol} not found, trying to select...")
    ...
    if result.retcode != mt5.TRADE_RETCODE_DONE:
        logging.error(f"MT5 order_send failed for {symbol} with retcode={result.retcode}")
        mt5.shutdown()
        return None

    logging.info(f"✅ MT5 Trade Executed Successfully! Ticket ID: {result.order} ({action} {symbol})")
    mt5.shutdown()
    return result.order
...
def calculate_dynamic_lot(ticker, sl_pips):
    ...
    account_balance = 10000.0  # Default base balance override if info unavailable
    if mt5.initialize():
    ...
    final_lot = max(0.01, min(calculated_lot, 1.0))  # Capped safely for demo/liveing
    return final_lot
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if args and args[0] == BOT_PASSCODE:
        ...
        await update.message.reply_text("🔓 **Passcode accepted!** Kings™ MT5 Engine active.", parse_mode="Markdown")
...
async def status_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = (
        f"📊 **KINGS™ MT5 ENGINE STATUS**\n"
        f"━━━━━━━━━━━━━━━━━━━\n"
        f"⚙️ **Circuit Breaker:** {status_msg}\n"
        f"🔌 **MT5 Terminal Link:** Connected\n"
    )
...
async def handle_text_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id in authorized_users:
        await update.message.reply_text("🟢 **Kings™ MT5 Engine is running.** Use `/status` to check engine health.", parse_mode="Markdown")
    else:
...
async def heartbeat_loop(app):
    while True:
        try:
            ...
            msg_text = f"{status_icon} *[Bot Heartbeat]* Kings™ MT5 Engine Status: {status_msg} ({formatted_wat})"
        ...
        try:
            ...
            for ticker, label in active_assets.items():
                ...
                if sig and last_signals.get(ticker) != sig:
                    ...
                    now_wat = datetime.datetime.now(datetime.timezone.utc)
                    ...
                    # Execute Trade directly on MT5 Terminal!
                    ticket_id = execute_mt5_trade(label, sig, volume=rec_lot, sl_price=sl, tp_price=tp)
                    exec_status_str = f"⚡ **MT5 TRADE PLACED (Ticket: {ticket_id})**" if ticket_id else "⚠️ **MT5 Execution Failed**"
                    ...
                    public_channel_text = (
                        ...
                        f"🕒 **Time:** `{time_sent_str} WAT`"
                    )

                    admin_preview_text = (
                        f"📋 **NEW SIGNAL & MT5 AUTO-TRADE**\n"
                        f"━━━━━━━━━━━━━━━━━━━\n"
                        f"{exec_status_str}\n\n"
...
def main():
    ...
    logging.info("Kings™ MT5 Auto Engine Active...")
